...
PidTuner::PidTuner(float ip, float ii, float id, float Sp, float Si, float Sd) {
    ...
    _overScale = 2;  // the step is divided by overscale each time it overshoots
                     // the target
}
...
void PidTuner::startCycle() {
    ...
    if (_testNum == _testSets.size()) {
        ...
        _testSets.push_back(
            PidSet(_currentPid.p + _pScale, _currentPid.i, _currentPid.d));
        ...
        _testSets.push_back(
            PidSet(_currentPid.p, _currentPid.i + _iScale, _currentPid.d));
        ...
        _testSets.push_back(
            PidSet(_currentPid.p, _currentPid.i, _currentPid.d + _dScale));
...
void PidTuner::run(float err) { _currentPid.score += fabs(err); }
...
bool PidTuner::endCycle() {
    ...
    std::cout << _currentPid.p << " | " << _currentPid.i << " | "
              << _currentPid.d << std::endl;
    std::cout << _currentPid.score << std::endl;

    if (_testNum == _testSets.size()) {
        std::cout << "---------------------------" << std::endl;

        // finished testing set, determine best pid, then check if we need more
        // tests
        _cycles += 1;
        ...
        _bestPid =
            *(std::min_element(std::begin(_testSets), std::end(_testSets)));

        // Don't actually need to include bestPid in testSet
        _testSets.pop_back();

        // Scales the test step down and reverses the direction
        //_pScale *= _testSets[0].score > _testSets[1].score ? 1 :
        //-1/_overScale;
        //_iScale *= _testSets[0].score > _testSets[2].score ? 1 :
        //-1/_overScale;
        //_dScale *= _testSets[0].score > _testSets[3].score ? 1 :
        //-1/_overScale;

        _currentPid = _bestPid;

        std::cout << "Best: " << _bestPid.p << " | " << _bestPid.i << " | "
                  << _bestPid.d << " | " << _bestPid.score << std::endl;
        std::cout << "---------------------------" << std::endl;
